...
class FileData:
    ...
    def from_dict(file_dict: dict) -> "FileData":
        ...
        file_type = file_dict.get(
            "type", "unknown"
        )  # Default type can be 'unknown' or any other placeholder
        url = file_dict.get("url", "")
        return FileData(name, file_type, url)

    def to_dict(self):
        return {"name": self.name, "type": self.file_type, "url": self.url}
...
class ResultsData:
    ...
    def from_dict(data_dict: dict) -> "ResultsData":
        ...
        status = response.get(
            "status", ""
        )  # Default to empty string if 'status' is not provided
        print("status: " + str(status))
        return ResultsData(message_id, files, error, logs, message, status)
...
class RegisterData:
    def __init__(self, master_token: str, name: str, description: str, status: int):
...
class Param:
    def __init__(
        ...
        max: int = None,
        step: int = None,
        options: List[Union[str, int, float]] = None,
    ):
        self.name = name
        self.type = type
    ...
    def from_dict(msg_dict: dict) -> "Message":
        if msg_dict["type"] == "contract":
            return Message(
                msg_dict["token"],
                msg_dict["type"],
                ContractData.from_dict(msg_dict["data"]),
            )
        elif msg_dict["type"] == "action":
            return Message(
                msg_dict["token"],
                msg_dict["type"],
                ActionData.from_dict(msg_dict["data"]),
            )
        elif msg_dict["type"] == "register":
            return Message(
                msg_dict["token"],
                msg_dict["type"],
                RegisterData.from_dict(msg_dict["data"]),
            )
        elif msg_dict["type"] == "results":
            return Message(
                msg_dict["token"],
                msg_dict["type"],
                ResultsData.from_dict(msg_dict["data"]),
            )
        else:
